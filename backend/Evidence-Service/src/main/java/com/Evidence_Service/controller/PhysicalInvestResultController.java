...
public class PhysicalInvestResultController {
    ...
    public ApiResponse<PhysicalInvestResultDTO> createPhysicalInvestResult(@PathVariable String evidenceId, @RequestBody PhysicalInvestResultDTO dto) {
        return ApiResponse.<PhysicalInvestResultDTO>builder().code(201).message("Created").data(physicalInvestResultService.addPhysicalInvestResult(evidenceId, dto)).build();
    }

    @GetMapping
    @PreAuthorize("hasAuthority('VIEW_PHYSICAL_RESULT')")
    @Operation(summary = "Get all physical investigation results")
    public ApiResponse<Page<PhysicalInvestResultDTO>> getAllPhysicalInvestResults(@PathVariable String evidenceId,
    ...
    public ApiResponse<PhysicalInvestResultDTO> getPhysicalInvestResultById(@PathVariable String resultId) {
        return ApiResponse.<PhysicalInvestResultDTO>builder().code(200).message("Fetched").data(physicalInvestResultService.getPhysicalInvestByResultId(resultId)).build();
    }

    @PutMapping("/{resultId}")
    @PreAuthorize("hasAuthority('EDIT_PHYSICAL_RESULT')")
    @Operation(summary = "Update physical investigation result")
    public ApiResponse<PhysicalInvestResultDTO> updatePhysicalInvestResult(@PathVariable String evidenceId, @PathVariable String resultId, @RequestBody PhysicalInvestResultDTO dto) {
        return ApiResponse.<PhysicalInvestResultDTO>builder().code(200).message("Updated").data(physicalInvestResultService.updatePhysicalInvest(evidenceId, resultId, dto)).build();
    }

    @DeleteMapping("/{resultId}")
    @PreAuthorize("hasAuthority('DELETE_PHYSICAL_RESULT')")
    @Operation(summary = "Delete physical investigation result")
    public ApiResponse<Void> deletePhysicalInvestResult(@PathVariable String resultId) {
